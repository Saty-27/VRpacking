  ...
  useEffect(() => {
    Promise.all([
      api.get('/pages').then(r => {
        const pg = r.data.find(p => p.slug === '/about-us');
        if (pg && pg.isActive === false) {
          setPageActive(false);
        }
      }),
      api.get('/sections/about').then(r => {
        setSections(r.data);
      })
    ]).catch(() => {})
    ...
    <>
        ...
        <section className="page-hero" style={{ backgroundImage: 'linear-gradient(135deg, rgba(17,24,39,0.92), rgba(17,24,39,0.92)), url("/about_banner.png")' }}>
          <div className="container">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              <h1>{getSection('hero').title || 'About VR Packaging Solutions'}</h1>
              <p>{getSection('hero').subtitle || 'Reliable industrial, VCI, and seaworthy packaging solutions.'}</p>
      ...
      {hasSection('expertise') && (
        <section className="section">
          <div className="container">
            <div className="section-header">
              <h2>{expertise.title || 'What We Do'}</h2>
              <p className="subtitle">{expertise.subtitle || 'Our packaging solutions cover a wide range of industrial and export requirements.'}</p>
            </div>
        ...
        <section className="section section-gradient">
          <div className="container" style={{ textAlign: 'center' }}>
            ...
            <Link to="/contact-us" className="btn btn-primary btn-lg" style={{ borderRadius: 4 }}>
              {cta.buttons?.[0]?.text || 'Contact Us Today'} <FaArrowRight />
            </Link>
          </div>
        </section>
      )}
